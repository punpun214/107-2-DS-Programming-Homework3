...
loss1Companies <- finStatement$公司名稱[a]
...
c <- finStatement$`營業利益（損失）` < 0 & finStatement$`基本每股盈餘（元）` > 0
only1LossCompanies <- finStatement$公司名稱[b | c]
...
# df_oneShow
stringsAsFactors=F
df_oneShow <- list(title = exhibitionList[[1]][["title"]] , time = exhibitionList[[1]][["showInfo"]][[1]][["time"]] , location = exhibitionList[[1]][["showInfo"]][[1]][["location"]] , locationName = exhibitionList[[1]][["showInfo"]][[1]][["locationName"]] , price = exhibitionList[[1]][["showInfo"]][[1]][["price"]] , endTime = exhibitionList[[1]][["showInfo"]][[1]][["endTime"]] , sourceWebName = exhibitionList[[1]][["sourceWebName"]])
df_oneShow <- as.data.frame(df_oneShow) 
class(df_oneShow)
names(df_oneShow)
df_oneShow
```
...
```{r ans25}
# totalCount
e <- exhibitionList["Name"] == "sourceWebName"
e
pretotalCount <- exhibitionList[e,3] == "全國藝文活動資訊系統"
totalCount <- sum(pretotalCount)
totalCount
```
